"""
Transformer Training + MPC Evaluation Pipeline (v2 - fast).

Key optimization: sample once per day (96 steps apart) instead of every
15-minute step. This reduces training samples from 137K to ~1400 per province,
making training feasible on M4 MPS in minutes, not hours.

Usage:
    PYTHONPATH=. python3 scripts/11_transformer_train.py --province shandong
    PYTHONPATH=. python3 scripts/11_transformer_train.py --all
"""
...
from forecast.transformer_config import get_config, TransformerConfig
...
# ============================================================
# Fast Dataset: one sample per day
# ============================================================

class EPFDatasetV2(Dataset):
    """
    Full-coverage dataset with 8-channel sequence features.

    Path A sequence channels (all normalized):
      0: rt_price       — core target history
      1: da_price        — day-ahead price
      2: da_rt_spread    — basis signal
      3: load_norm       — demand side
      4: renewable_pen   — supply side (renewable penetration)
      5: net_load_norm   — net demand
      6: wind_ratio      — wind vs solar mix
      7: solar_ratio     — solar vs wind mix

    This gives the Transformer rich supply/demand context at every timestep,
    not just price history. stride=1 for maximum data on GPU.
    """

    # Sequence feature columns (from features.py)
    SEQ_COLS = [
    ...
    def __init__(self, df, exo_cols, config, augment=False, stride=1):
        ...
        # Precompute sequence features (N, 8)
        seq_data = []
        ...
        self.seq = np.stack(seq_data, axis=-1)  # (N, 8)
        ...
        # Valid indices
        self.indices = np.arange(W, N - T, stride)
        logger.info(f"    Dataset: {len(self.indices)} samples (stride={stride}, N={N})")

        # Precompute rolling stats
        norm_w = config.norm_window
        kernel = np.ones(norm_w, dtype=np.float32) / norm_w
        self.rolling_mean = np.convolve(self.rt, kernel, mode='same')
        rt_sq = self.rt ** 2
        mean_sq = np.convolve(rt_sq, kernel, mode='same')
        mean_val = self.rolling_mean.copy()
        self.rolling_std = np.sqrt(np.maximum(mean_sq - mean_val ** 2, 0) + 1e-6).astype(np.float32)
        self.rolling_mean = self.rolling_mean.astype(np.float32)
    ...
    def __getitem__(self, i):
        ...
        if self.augment and self.config.jitter_steps > 0:
            ...
            idx = max(W, min(idx + jitter, len(self.rt) - T - 1))

        mean = self.rolling_mean[idx]
        std = max(self.rolling_std[idx], 1.0)

        # Build 8-channel sequence window (W, 8)
        s = idx - W
        seq_win = self.seq[s:idx].copy()  # (W, 8)

        # Normalize price channels (0,1,2) with adaptive stats
        # Non-price channels (3-7) are already normalized in features.py
        seq_win[:, 0] = (seq_win[:, 0] - mean) / std  # rt_price
        seq_win[:, 1] = (seq_win[:, 1] - mean) / std  # da_price
        seq_win[:, 2] = seq_win[:, 2] / std            # spread (already a difference)
        ...
        return (
            ...
            torch.from_numpy(self.exo[idx].copy()),
...
def extract_arrays(df):
    rt = df["rt_price"].fillna(0).values.astype(np.float32)
    da = df["da_price"].fillna(0).values if "da_price" in df.columns else np.zeros(len(df), dtype=np.float32)
    da = da.astype(np.float32)
    exo = df[EXO_COLS].fillna(0).values.astype(np.float32)
    return rt, da, exo
...
def train_transformer(province, df_train, df_val, config):
    logger.info(f"\n--- Training Transformer for {province} ---")
    logger.info(f"  Config: embed={config.embed_dim}, heads={config.n_heads}, "
                f"layers={config.n_layers}, ff={config.ff_dim}")
    ...
    train_ds = EPFDatasetV2(df_train, EXO_COLS, config, augment=True, stride=1)
    val_ds = EPFDatasetV2(df_val, EXO_COLS, config, augment=False, stride=96)

    logger.info(f"  Train samples: {len(train_ds)}, Val samples: {len(val_ds)}")

    device = torch.device(config.device if torch.cuda.is_available() or (hasattr(torch.backends, "mps") and torch.backends.mps.is_available()) else "cpu")
    model = TransformerEPF(config).to(device)
    ...
    for epoch in range(config.max_epochs):
        ...
        model.train()
        train_losses = []
        for price_seq, exo, target, ns in train_loader:
            price_seq, exo, target = price_seq.to(device), exo.to(device), target.to(device)
            optimizer.zero_grad()
        ...
        val_maes = []
        with torch.no_grad():
            for price_seq, exo, target, ns in val_loader:
                price_seq, exo, target, ns = price_seq.to(device), exo.to(device), target.to(device), ns.to(device)
                pred = model(price_seq, exo)
    ...
    save_path = MODEL_DIR / f"transformer_{province}.pt"
    torch.save({"model_state": model.state_dict(), "config": config}, save_path)
    logger.info(f"  Saved: {save_path}")

    forecaster = TransformerForecaster(config, model)
    return forecaster


# ============================================================
# MPC Evaluation (same as 09_mpc_eval.py for fair comparison)
# ============================================================

def evaluate(province, df_train, df_val, df_test, transformer_fc):
    ...
    # Build full sequence features (8 channels) for Transformer inference
    seq_cols = EPFDatasetV2.SEQ_COLS
    ...
    logger.info("\n--- Transformer MPC ---")
    t0 = time.time()
    transformer_fc._full_prices = full_prices
    transformer_fc._full_da_prices = full_da
    transformer_fc._full_exo = full_exo
    transformer_fc._full_seq = full_seq

    class TFWrap:
        def predict(self, f, idx, horizon=96):
            return transformer_fc.predict(f, test_offset + idx, horizon)

    tf_ctrl = MPCController(TFWrap(), battery, continuous=True, daily_profile=daily_profile)
    tf_r = simulate_mpc(tf_ctrl, features_test, prices_test, battery, replan_every=4, log_every=90)
    logger.info(f"  Revenue: {tf_r['revenue']:,.0f}  ({time.time() - t0:.0f}s)")

    # Summary
    test_d = len(prices_test) // 96
    ann = lambda r: r / test_d * 365
    tf_vs_lgbm = (tf_r["revenue"] - lgbm_r["revenue"]) / abs(lgbm_r["revenue"]) * 100
...
def main():
    ...
    for prov in provinces:
        ...
        fc = train_transformer(prov, df_train, df_val, config)
        r = evaluate(prov, df_train, df_val, df_test, fc)
        results.append(r)

    if len(results) > 1:
        logger.info(f"\n{'=' * 80}")
        logger.info(f"  FINAL — Transformer vs LightGBM")
        logger.info(f"{'=' * 80}")
        for r in results:
            logger.info(f"  {r['province']:<12} LGBM={r['lgbm_mpc']:>12,.0f}  TF={r['transformer_mpc']:>12,.0f}  diff={r['tf_vs_lgbm_pct']:>+6.1f}%")
        pd.DataFrame(results).to_csv(PROCESSED_DIR / "transformer_results.csv", index=False)
