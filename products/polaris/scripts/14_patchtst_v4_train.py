"""
PatchTST-EPF V4 Training + MPC Evaluation.

V4 = V3b architecture + causal weather/supply-demand features (39-dim exo).

Usage:
    PYTHONPATH=. python3 scripts/14_patchtst_v4_train.py --province shandong
    PYTHONPATH=. python3 scripts/14_patchtst_v4_train.py --all
"""
...
from forecast.transformer_config import get_config_v4, TransformerConfig
...
def evaluate(province, df_train, df_val, df_test, model, config):
    ...
    logger.info(f"\n{'=' * 80}")
    logger.info(f"  {province.upper()} — {test_d} test days (PatchTST V4)")
    logger.info(f"{'=' * 80}")
    ...
    for name, rev in [
        ...
        ("PatchTST V4 MPC", ptst_r["revenue"]),
    ...
    logger.info(f"\n  >>> PatchTST V4 vs LightGBM: {ptst_vs_lgbm:+.1f}% <<<")
...
def main():
    ...
    for prov in provinces:
        logger.info(f"\n{'#' * 80}\n# {prov.upper()} (PatchTST V4)\n{'#' * 80}")
        config = get_config_v4(prov)
    ...
    if len(results) > 1:
        logger.info(f"\n{'=' * 80}")
        logger.info(f"  FINAL — PatchTST V4 vs LightGBM")
        logger.info(f"{'=' * 80}")
        for r in results:
