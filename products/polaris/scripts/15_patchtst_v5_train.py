"""
PatchTST-EPF V5 Training + MPC Evaluation + Ensemble.

V5 = V4 + Huber loss + linear skip connection + LightGBM ensemble.

Changes from V4:
  - Huber loss (robust to price spikes) replaces MSE
  - Linear skip connection: lag_96 → output directly (El Mahtout & Ziel 2025)
  - Ensemble evaluation: alpha * LightGBM + (1-alpha) * PatchTST

Usage:
    PYTHONPATH=. python3 scripts/15_patchtst_v5_train.py --province shandong
    PYTHONPATH=. python3 scripts/15_patchtst_v5_train.py --all
"""
...
EXO_COLS = FEATURE_COLS_V4  # 39 features (31 base + 8 causal)
...
def train_patchtst(province, df_train, df_val, config):
    logger.info(f"\n--- Training PatchTST-EPF V5 for {province} ---")
    ...
    logger.info(f"  V5: loss={config.loss_type}, skip={config.use_skip_connection}")
    ...
    # V5: Configurable loss function
    if config.loss_type == "huber":
        criterion = nn.HuberLoss(delta=config.huber_delta)
        logger.info(f"  Loss: Huber (delta={config.huber_delta})")
    elif config.loss_type == "mae":
        criterion = nn.L1Loss()
        logger.info(f"  Loss: MAE (L1)")
    else:
...
def evaluate(province, df_train, df_val, df_test, model, config):
    ...
    # 5. Ensemble: alpha * LightGBM + (1-alpha) * PatchTST
    logger.info("\n--- Ensemble Search (LGBM + PatchTST V5) ---")
    best_ens_rev = -np.inf
    best_alpha = 0.5
    ensemble_results = {}

    for alpha_pct in range(30, 85, 5):  # alpha from 0.3 to 0.8
        ...
        class EnsembleWrap:
            def __init__(self, a):
                self._alpha = a
            def predict(self, f, idx, horizon=96):
                p_lgbm = lgbm.predict(f, test_offset + idx, horizon)
                p_ptst = forecaster.predict(f, test_offset + idx, horizon)
                return self._alpha * p_lgbm + (1 - self._alpha) * p_ptst

        ens_ctrl = MPCController(EnsembleWrap(alpha), battery, continuous=True, daily_profile=daily_profile)
        ens_r = simulate_mpc(ens_ctrl, features_test, prices_test, battery, replan_every=4)
        ensemble_results[alpha] = ens_r["revenue"]
        vs_lgbm = (ens_r["revenue"] - lgbm_r["revenue"]) / abs(lgbm_r["revenue"]) * 100
        logger.info(f"  alpha={alpha:.2f}: Revenue={ens_r['revenue']:,.0f} (vs LGBM: {vs_lgbm:+.1f}%)")

        if ens_r["revenue"] > best_ens_rev:
            best_ens_rev = ens_r["revenue"]
            best_alpha = alpha

    ens_vs_lgbm = (best_ens_rev - lgbm_r["revenue"]) / abs(lgbm_r["revenue"]) * 100
    logger.info(f"  Best ensemble: alpha={best_alpha:.2f}, Revenue={best_ens_rev:,.0f} (vs LGBM: {ens_vs_lgbm:+.1f}%)")
    ...
    logger.info(f"\n{'=' * 80}")
    logger.info(f"  {province.upper()} — {test_d} test days (PatchTST V5)")
    logger.info(f"{'=' * 80}")
    logger.info(f"  {'Method':<30} {'Revenue':>14} {'Annual':>14} {'vs TH':>10}")
    logger.info(f"  {'-' * 30} {'-' * 14} {'-' * 14} {'-' * 10}")

    for name, rev in [
        ("Threshold", th["revenue"]),
        ("LightGBM MPC", lgbm_r["revenue"]),
        ("PatchTST V5 MPC", ptst_r["revenue"]),
        (f"Ensemble (a={best_alpha:.2f})", best_ens_rev),
        ...
        logger.info(f"  {name:<30} {rev:>14,.0f} {ann(rev):>14,.0f} {vs:>+9.1f}%")

    logger.info(f"\n  >>> PatchTST V5 vs LightGBM: {ptst_vs_lgbm:+.1f}% <<<")
    logger.info(f"  >>> Ensemble vs LightGBM: {ens_vs_lgbm:+.1f}% <<<")

    return {
        "province": province, "test_days": test_d,
        "threshold": th["revenue"], "lgbm_mpc": lgbm_r["revenue"],
        "patchtst_mpc": ptst_r["revenue"],
        "ensemble_mpc": best_ens_rev, "best_alpha": best_alpha,
        "oracle_disc": oracle_disc["revenue"], "oracle_cont": oracle_cont["revenue"],
        "ptst_vs_lgbm_pct": ptst_vs_lgbm, "ens_vs_lgbm_pct": ens_vs_lgbm,
    }
...
def main():
    ...
    for prov in provinces:
        logger.info(f"\n{'#' * 80}\n# {prov.upper()} (PatchTST V5)\n{'#' * 80}")
        config = get_config_v5(prov)
    ...
    if len(results) > 1:
        logger.info(f"\n{'=' * 80}")
        logger.info(f"  FINAL — PatchTST V5 + Ensemble vs LightGBM")
        logger.info(f"{'=' * 80}")
        for r in results:
            logger.info(f"  {r['province']:<12} LGBM={r['lgbm_mpc']:>12,.0f}  V5={r['patchtst_mpc']:>12,.0f}({r['ptst_vs_lgbm_pct']:>+.1f}%)  ENS={r['ensemble_mpc']:>12,.0f}({r['ens_vs_lgbm_pct']:>+.1f}%,a={r['best_alpha']:.2f})")
        pd.DataFrame(results).to_csv(PROCESSED_DIR / "patchtst_v5_results.csv", index=False)
